...
<template>
  <div v-if="isNoPagesAsked" style="text-align: center">
    ...
    <p v-if="this.reqState.data.length === 0">
      No pages created. Make the first one by adding a path to the url !
    </p>
    <div v-for="item of this.reqState.data">
      <a :href="item.pathName" style="color: #c850c0; font-weight: 600">{{
        item.pathName
      }}</a>
    </div>
  </div>
  <div v-if="!isNoPagesAsked && isDisplaying">
    <button
      @click="makeEdition()"
      style="background-color: #4158d0; font-weight: 600"
    >
      <font-awesome-icon
